...
func (e *RootEntry) Append(entry Entry) {
	...
	for _, child := range *e {
		...
		case *DirEntry:
			...
			if strings.HasPrefix(parent, strings.TrimSuffix(normalizePath(dir.Filepath), "/")+"/") {
				dir.Append(entry)
				return
			}
		default:
...
func (e *DirEntry) Append(entry Entry) {
	...
	for _, child := range e.Children {
		...
		case *DirEntry:
			...
			if strings.HasPrefix(parent, strings.TrimSuffix(normalizePath(dir.path()), "/")+"/") {
				dir.Append(entry)
				return
			}
		default:
