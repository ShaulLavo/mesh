...
front door, sessions over SSH, read-only SFTP and SCP, and named reverse tunnels are complete.
Target-authorized wake, automatic LAN sender selection, Linux/macOS sleep
inhibition, and workspace crash recovery are also implemented. Physical
power-loss acceptance remains an operator check.
...
- `internal/sshfs` — read-only SFTP and SCP over the live serving registry,
  synthetic service directories, and shared root confinement (T16)
...
Verified 2026-09-06 after T16: `go test -race ./...`, `go vet ./...`, clean module
checks, all 37 integration scripts, and `scripts/check-t16.sh` passed. Stock
OpenSSH SFTP, modern SCP, and legacy SCP completed recursive downloads. The T25
dependency guard passed. Review regressions cover SCP sibling placement, literal
percent filenames, symlink resolution order, and descriptor-based FSTAT. A
2026-09-06 review also fixed legacy SCP closing before the client acknowledged
the completed transfer; 80 repeated stock-client downloads passed afterward. The
dependency checker verifies the two local patches against pinned upstream
archives. A GUI file-manager mount remains an unperformed manual check. Full
lint retains the eleven existing bootstrap and CLI findings.
...
| 9 SSH front door | T15, T16, T17, T18 | complete |
...
T25 now captures and resumes exact
