#!/usr/bin/env bash
# Signed staging bundles persist without becoming serving certificates; live
# bundles hot-rotate the origin's loopback HTTPS listener without a restart.
set -uo pipefail
...
served_fingerprint() {
  openssl s_client -connect "127.0.0.1:$HTTPS_PORT" -servername pc.mesh.shaulavo.dev -showcerts </dev/null 2>/dev/null |
...
BODY=$(curl --noproxy '*' --fail --silent --max-time 2 --cacert "$TEST_ROOT/live-one.crt" \
  --resolve "pc.mesh.shaulavo.dev:$HTTPS_PORT:127.0.0.1" "https://pc.mesh.shaulavo.dev:$HTTPS_PORT/site/") || fail "HTTPS service request"
...
TERMINAL_STATUS=$(curl --noproxy '*' --silent --max-time 2 --cacert "$TEST_ROOT/live-one.crt" \
  --resolve "pc.mesh.shaulavo.dev:$HTTPS_PORT:127.0.0.1" \
  --header 'Connection: Upgrade' --header 'Upgrade: websocket' \
  --output /dev/null --write-out '%{http_code}' "https://pc.mesh.shaulavo.dev:$HTTPS_PORT/mesh") || fail "HTTPS terminal isolation request"
...
echo "PASS: staging stayed non-serving and live TLS hot-rotated on the origin daemon"
